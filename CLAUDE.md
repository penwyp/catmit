...
- **`cmd/squash_draft.go`** - Squash draft command implementation (editor mode)
- **`cmd/squash_history.go`** - Squash history command implementation (rebase mode)
- **`cmd/pr.go`** - Pull request command implementation
- **`cmd/check_auth.go`** - Authentication status checking for all remotes
...
### Squash Draft workflow (Editor Mode)
```bash
catmit squash-draft        # Default mode (opens editor) with TUI
catmit squash-draft -y     # Skip confirmation, output directly
catmit squash-draft -l zh  # Generate Chinese commit message
catmit squash-draft -t 60  # 60 second timeout
```

The squash-draft command helps consolidate multiple commit messages into a single, comprehensive commit message. It:
...
### Squash History workflow (Rebase Mode)
```bash
catmit squash-history      # Analyze and squash unpushed commits
catmit squash-history -y   # Auto-confirm without TUI
catmit squash-history -l zh # Generate Chinese commit message
catmit squash-history -t 60 # 60 second timeout
```

The squash-history command automates the process of squashing unpushed commits:
- Automatically detects unpushed commits against the base branch (main/master)
- Uses LLM to generate a consolidated commit message from all unpushed commits
- Creates a backup branch before making changes for safety
- Performs interactive rebase with the generated message
- Provides TUI interface for monitoring progress and confirmation

### Pull Request workflow
```bash
catmit pr                  # Create PR with AI-generated description
catmit pr --title "feat: add auth" # Custom title
catmit pr --draft          # Create as draft PR
catmit pr --remote upstream --base develop # Specify remote and base
catmit pr --template=false # Skip template usage
catmit check-auth         # Check authentication status for all remotes
```

### PR Feature Details
- **Multi-Platform Support**: GitHub (`gh`), GitLab (`glab`), Gitea (`tea`), with auto-detection
- **Template Support**: Customizable PR templates via config file or repository templates
- **AI-Generated Descriptions**: Uses recent commits and changes to create meaningful PR descriptions
- **Smart Push Detection**: Only pushes if branch doesn't exist remotely
- **Error Handling**: Shows existing PR URL if PR already exists
- **Auth Check**: Validates CLI installation and authentication
- **Flexible Options**: Custom title, draft mode, remote/base specification
