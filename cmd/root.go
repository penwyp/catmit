...
func run(cmd *cobra.Command, args []string) error {
	...
	if isPRRequested() && !flagYes && !flagDryRun {
		col := collectorProvider()
		_, err := col.ComprehensiveDiff(ctx)
		if err != nil && errors.Is(err, collector.ErrNoDiff) {
			...
			if err != nil {
				...
				if errors.As(err, &prExists) {
					...
					if prExists.URL != "" {
						...
						_, _ = fmt.Fprintln(cmd.OutOrStdout(), "Please check your Git hosting platform for the existing PR")
					}
					return nil
				}
		...
		// If there's an error other than NoDiff or there are changes, fall through to normal flow
	}
