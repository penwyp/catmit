...
catmit 支持 YAML 配置文件进行高级自定义：

**配置文件位置**：`~/.config/catmit/config.yaml`
...
**自定义模板配置**：
创建 `~/.config/catmit/config.yaml` 来定义自定义 PR 模板：

```yaml
llm:
  api_key: "sk-xxxx"  # 也可以使用环境变量
  api_url: "https://api.deepseek.com/v1/chat/completions"
  model: "deepseek-chat"
