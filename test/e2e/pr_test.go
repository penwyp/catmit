...
func TestE2E_PRCreation_GitHub(t *testing.T) {
	...
	cmd = exec.Command("git", "commit", "-m", "feat: add new feature for testing PR creation")
	cmd.Dir = repo
	err = cmd.Run()
	require.NoError(t, err)

	// Run catmit pr command
	catmitCmd := exec.Command(bin, "pr", "-y")
	catmitCmd.Dir = repo
...
case "$args" in
  ...
  "login list")
    echo '┌─────────┬──────────────────┬──────────┬──────────┬──────────┬───────────────┐'
    echo '│ NAME    │ URL              │ SSH HOST │ USER     │ DEFAULT  │ ACCESS TOKEN  │'
    echo '├─────────┼──────────────────┼──────────┼──────────┼──────────┼───────────────┤'
    echo '│ Default │ gitea.example.com│          │ testuser │ true     │ ***********   │'
    echo '└─────────┴──────────────────┴──────────┴──────────┴──────────┴───────────────┘'
    exit 0
    ;;
  pr\ create*)
...
`
	...
	// Commit the changes first
	cmd = exec.Command("git", "commit", "-m", "fix: resolve critical bug in authentication")
	cmd.Dir = repo
	err = cmd.Run()
	require.NoError(t, err)
...
func TestE2E_PRCreation_NoChanges(t *testing.T) {
	...
	// Mock LLM server for PR description generation
	llmServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		response := map[string]interface{}{
			"choices": []map[string]interface{}{
				{
					"message": map[string]string{
						"content": "feat: update branch with minimal changes",
					},
				},
			},
		}
	...
	ghMock := mockGitHubCLI(t, map[string]string{
		"auth status": "Logged in to github.com as testuser",
		"pr create --title [Chore] feat: update branch with minimal changes --body feat: update branch with minimal changes --base master": "https://github.com/owner/repo/pull/456",
	})

	repo := setupPRTestRepo(t, "https://github.com/owner/repo.git")

	// Run catmit pr command with no changes
	catmitCmd := exec.Command(bin, "pr", "-y")
	catmitCmd.Dir = repo
	catmitCmd.Env = append(os.Environ(),
		"CATMIT_LLM_API_KEY=test-key",
	...
	// When pr command is used, it creates PR even with no changes
...
`
	...
	// Commit the changes first
	cmd = exec.Command("git", "commit", "-m", "feat: add duplicate feature")
	cmd.Dir = repo
	err = cmd.Run()
	require.NoError(t, err)
...
func TestE2E_PRCreation_CLINotInstalled(t *testing.T) {
	...
	// Commit the changes first
	cmd = exec.Command("git", "commit", "-m", "feat: test cli not found")
	cmd.Dir = repo
	err = cmd.Run()
	require.NoError(t, err)

	// Run catmit pr command with limited PATH to simulate CLI not found
	catmitCmd := exec.Command(bin, "pr", "-y")
	catmitCmd.Dir = repo
	...
	require.Contains(t, out.String(), "required CLI tool is not installed")
}
...
func TestE2E_PRCreation_Timeout(t *testing.T) {
	...
	// Commit the changes first
	cmd = exec.Command("git", "commit", "-m", "feat: timeout test")
	cmd.Dir = repo
	err = cmd.Run()
	require.NoError(t, err)

	// Run catmit pr command with short timeout
	catmitCmd := exec.Command(bin, "pr", "-y", "-t", "1")
	catmitCmd.Dir = repo
	catmitCmd.Env = append(os.Environ(),
		"CATMIT_LLM_API_KEY=test-key",
		"CATMIT_LLM_API_URL="+llmServer.URL,
