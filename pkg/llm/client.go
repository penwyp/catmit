	...
	if c.logger != nil {
		...
		if oaiProvider, ok := c.provider.(*OpenAICompatibleProvider); ok {
			...
			c.logger.Debug("LLM API Request Details",
				...
				zap.Int("system_prompt_length", len(systemPrompt)),
				zap.Int("user_prompt_length", len(userPrompt)))
		} else if anthropicProvider, ok := c.provider.(*AnthropicProvider); ok {
			c.logger.Debug("LLM API Request Details",
				zap.String("provider", "anthropic"),
				zap.String("api_url", anthropicProvider.apiURL),
				zap.String("api_key_masked", maskAPIKey(anthropicProvider.apiKey)),
				zap.String("model", anthropicProvider.model),
				zap.String("system_prompt_preview", truncateForLog(systemPrompt, 100)),
				zap.String("user_prompt_preview", truncateForLog(userPrompt, 100)),
				zap.Int("system_prompt_length", len(systemPrompt)),
				zap.Int("user_prompt_length", len(userPrompt)))
		} else if geminiProvider, ok := c.provider.(*GeminiProvider); ok {
			c.logger.Debug("LLM API Request Details",
				zap.String("provider", "gemini"),
				zap.String("api_url", geminiProvider.apiURL),
				zap.String("api_key_masked", maskAPIKey(geminiProvider.apiKey)),
				zap.String("model", geminiProvider.model),
				zap.String("system_prompt_preview", truncateForLog(systemPrompt, 100)),
				zap.String("user_prompt_preview", truncateForLog(userPrompt, 100)),
				zap.Int("system_prompt_length", len(systemPrompt)),
				zap.Int("user_prompt_length", len(userPrompt)))
		} else if ollamaProvider, ok := c.provider.(*OllamaProvider); ok {
			c.logger.Debug("LLM API Request Details",
				zap.String("provider", "ollama"),
				zap.String("api_url", ollamaProvider.apiURL),
				zap.String("model", ollamaProvider.model),
				zap.String("system_prompt_preview", truncateForLog(systemPrompt, 100)),
				zap.String("user_prompt_preview", truncateForLog(userPrompt, 100)),
				zap.Int("system_prompt_length", len(systemPrompt)),
				zap.Int("user_prompt_length", len(userPrompt)))
