...
func TestPredefinedErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			errType:    ErrTypeGit,
			retryable:  false,
			suggestion: "Please run this command in a Git repository",
		},
		{
		...
		},
		{
			...
			errType:    ErrTypeLLM,
			retryable:  false,
			suggestion: "Set the environment variable CATMIT_LLM_API_KEY",
		},
	}
...
func TestGetSuggestion(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test suggestion field directly
			var catmitErr *CatmitError
			if errors.As(tt.err, &catmitErr) {
				assert.Equal(t, tt.expected, catmitErr.Suggestion)
			} else {
