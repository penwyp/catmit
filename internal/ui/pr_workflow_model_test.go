...
// mockExtendedGitRunner implements pr.ExtendedGitRunner
type mockExtendedGitRunner struct {
	mock.Mock
}

func (m *mockExtendedGitRunner) GetRemoteURL(ctx context.Context, remote string) (string, error) {
	args := m.Called(ctx, remote)
	return args.String(0), args.Error(1)
}

func (m *mockExtendedGitRunner) GetCurrentBranch(ctx context.Context) (string, error) {
	args := m.Called(ctx)
	return args.String(0), args.Error(1)
}

func (m *mockExtendedGitRunner) GetCommitMessage(ctx context.Context, ref string) (string, error) {
	args := m.Called(ctx, ref)
	return args.String(0), args.Error(1)
}

func (m *mockExtendedGitRunner) GetDefaultBranch(ctx context.Context, remote string) (string, error) {
	args := m.Called(ctx, remote)
	return args.String(0), args.Error(1)
}

func (m *mockExtendedGitRunner) GetParentBranch(ctx context.Context, remote string) (string, error) {
	args := m.Called(ctx, remote)
	return args.String(0), args.Error(1)
}

func (m *mockExtendedGitRunner) Run(ctx context.Context, name string, args ...string) ([]byte, error) {
...
func createTestPRWorkflowModel() (*PRWorkflowModel, *mockPRCollector, *mockPRPromptBuilder, *mockPRClient, *mockPRCommitter, *mockExtendedGitRunner) {
	ctx := context.Background()
	collector := &mockPRCollector{}
	promptBuilder := &mockPRPromptBuilder{}
	client := &mockPRClient{}
	committer := &mockPRCommitter{}
	gitRunner := &mockExtendedGitRunner{}
	...
	return model, collector, promptBuilder, client, committer, gitRunner
}
...
func TestPRWorkflowModel_Init(t *testing.T) {
	model, collector, _, _, _, _ := createTestPRWorkflowModel()
...
func TestPRWorkflowModel_BuildPRPrompt(t *testing.T) {
	model, collector, promptBuilder, _, _, gitRunner := createTestPRWorkflowModel()
	
	// Setup mocks for GitRunner - CommitAnalyzer will call these
	gitRunner.On("GetCurrentBranch", mock.Anything).Return("feature-branch", nil)
	gitRunner.On("GetRemoteURL", mock.Anything, "origin").Return("https://github.com/test/repo.git", errors.New("remote not found"))
	gitRunner.On("Run", mock.Anything, "git", mock.Anything).Return([]byte(""), errors.New("no commits"))
	
	// Setup mocks for collector (fallback)
	collector.On("BranchName", mock.Anything).Return("feature-branch", nil)
	
	// Setup mocks for prompt builder
	promptBuilder.On("BuildPRSystemPrompt").Return("PR system prompt")
	promptBuilder.On("BuildPRUserPrompt", mock.Anything).Return("PR user prompt")
	...
	// The user prompt will be branch-based since we're simulating no commits
	assert.Contains(t, promptMsg.userPrompt, "feature-branch")
}

// Test loading phase progression
func TestPRWorkflowModel_LoadingPhase_Progression(t *testing.T) {
	model, collector, promptBuilder, client, _, gitRunner := createTestPRWorkflowModel()
	
	// Setup mocks for GitRunner - CommitAnalyzer will call these
	gitRunner.On("GetCurrentBranch", mock.Anything).Return("feature-branch", nil)
	gitRunner.On("GetRemoteURL", mock.Anything, "origin").Return("", errors.New("remote not found"))
	gitRunner.On("Run", mock.Anything, "git", mock.Anything).Return([]byte("sha1\nsha2"), nil).Maybe()
	
	// Setup mocks
	collector.On("BranchName", mock.Anything).Return("feature-branch", nil)
	promptBuilder.On("BuildPRSystemPrompt").Return("PR system prompt")
	promptBuilder.On("BuildPRUserPrompt", mock.Anything).Return("PR user prompt")
	client.On("GetCommitMessage", mock.Anything, "PR system prompt", mock.Anything).
...
func TestPRWorkflowModel_CommitPhase_NeedsPush(t *testing.T) {
	model, _, _, _, committer, _ := createTestPRWorkflowModel()
	model.phase = WorkflowPhaseReview
	model.message = "feat: PR title\n\nPR body"
...
func TestPRWorkflowModel_CompleteWorkflow(t *testing.T) {
	model, collector, _, _, committer, _ := createTestPRWorkflowModel()
...
func TestPRWorkflowModel_RenderPRPreviewContent_WithModel(t *testing.T) {
	...
	model.prPreview = NewEnhancedPRPreviewModel(prData, DefaultStyles(), CalculateContentWidth(model.width), 24)
