...
func (m *BaseModel) HandleKeyboard(msg tea.KeyMsg) tea.Cmd {
	switch msg.String() {
	case "esc", "ctrl+c":
		// For esc and ctrl+c, check if there's a cancel action first
		for i, action := range m.actions {
			if strings.ToUpper(action.Key) == "C" {
				m.selected = i
				return action.Handler()
			}
		}
		// If no cancel action, quit directly
		m.done = true
		return tea.Quit

	case "q", "Q":
		// For q/Q, always quit directly (unless there's a Q action)
		key := strings.ToUpper(msg.String())
		for i, action := range m.actions {
			if strings.ToUpper(action.Key) == key {
				m.selected = i
				return action.Handler()
			}
		}
