...
func TestBaseModel_HandleKeyboard(t *testing.T) {
	...
	actions := []Action{
		...
		{Key: "C", Label: "Cancel", Handler: func() tea.Cmd { 
			quitCalled = true
			// Note: In real workflow models, the handler would also set model.done = true
			// but we can't access model from here in the test
			return tea.Quit 
		}},
	}
	...
	tests := []struct {
		...
		{"Esc quits", "esc", 2, false, func() bool { return quitCalled }}, // Handler called but done not set by HandleKeyboard
		{"Ctrl+C quits", "ctrl+c", 2, false, func() bool { return quitCalled }}, // Handler called but done not set by HandleKeyboard
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create proper key message based on key type
			...
			cmd := model.HandleKeyboard(keyMsg)
			...
			if tt.name == "Esc quits" || tt.name == "Ctrl+C quits" || tt.name == "Q quits" || tt.name == "Letter key triggers action (uppercase)" {
				assert.NotNil(t, cmd)
			}
		})
	}
