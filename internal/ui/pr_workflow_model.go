...
type PRWorkflowModel struct {
	*BaseWorkflowModel
	...
	gitRunner   pr.ExtendedGitRunner // Add ExtendedGitRunner for commit analysis
	...
	// PR preview related
	prPreview     tea.Model // Changed to interface to support enhanced model
	prPreviewData PRPreviewData
	prURL         string

	// Template related
	useTemplate    bool
	rawLLMResponse string // Store raw LLM response for preview
}
...
func NewPRWorkflowModel(
	...
	cli clientInterface,
	com commitInterface,
	gitRunner pr.ExtendedGitRunner,
	lang string,
...
// HandleGlobalKeys handles global keyboard shortcuts with PR-specific behavior
func (m *PRWorkflowModel) HandleGlobalKeys(msg tea.KeyMsg) (bool, tea.Cmd) {
	if msg.String() == "ctrl+c" {
		m.err = context.Canceled
		// Set review decision for phases where it's relevant
		if m.phase == WorkflowPhaseReview || m.phase == WorkflowPhasePRPreview {
...
func (m *PRWorkflowModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case queryDoneMsg:
		...
		m.rawLLMResponse = msg.rawLLMResponse // Store raw response
	...
	case prPreviewReadyMsg:
		...
		// Use enhanced preview model instead of basic one
		m.prPreview = NewEnhancedPRPreviewModel(msg.data, DefaultStyles(), m.width, m.height)
...
func (m *PRWorkflowModel) renderPRPreviewContent() string {
	...
	// prPreview is now a tea.Model interface, so we can call View directly
...
func (m *PRWorkflowModel) updatePRPreview(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	// The enhanced preview model handles its own keyboard events
	if m.prPreview != nil {
		updatedPreview, cmd := m.prPreview.Update(msg)
		m.prPreview = updatedPreview
		if cmd != nil {
			return m, cmd
		}
	}
	
	// Let BaseModel handle action keys
...
func (m *PRWorkflowModel) preparePRPreview() tea.Cmd {
	return func() tea.Msg {
		...
		// Create template variables map (simplified for now)
		...
		prData := PRPreviewData{
			...
			UsingTemplate:  m.useTemplate,
			TemplateName:   "Default", // TODO: Get actual template name
			RawLLMResponse: m.rawLLMResponse,
...
func (m *PRWorkflowModel) buildPRPromptCmd() tea.Cmd {
	return func() tea.Msg {
		...
		analyzer := pr.NewCommitAnalyzer(m.gitRunner)
		
		// Analyze commits relative to the PR base branch
		analysisData, err := analyzer.AnalyzeForPR(m.ctx, m.prRemote, m.prBase)
		if err != nil {
			// Even if analysis fails, we can still try to use branch name
		...
		if analysisData != nil && len(analysisData.Commits) > 0 {
			...
			userPrompt = m.promptBuild.BuildPRUserPrompt(commitMessages)
			...
			userPrompt = "Based on the branch name '" + analysisData.BranchName + "', generate a PR title and description. This branch has no new commits yet, so infer the purpose from the branch name."
		} else {
			// Fallback: try to get branch name directly
			branchName, err := m.collector.BranchName(m.ctx)
			if err != nil {
				return errorMsg{err}
			}
			userPrompt = "Based on the branch name '" + branchName + "', generate a PR title and description. This branch has no new commits yet, so infer the purpose from the branch name."
		}
